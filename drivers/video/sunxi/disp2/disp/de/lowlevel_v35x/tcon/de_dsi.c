...
s32 dsi_start(u32 sel, enum __dsi_start_t func)
{
	...
	case DSI_START_HSD_DS:  /* lp11-(nop-deskew initial-hsd-dly-nop)----hscexit-end */
		dsi_dev[sel]->dsi_inst_jump_sel.dwval =
			...
			| DSI_INST_ID_NOP << (4 * DSI_INST_ID_DLY)
			| DSI_INST_ID_END << (4 * DSI_INST_ID_HSCEXIT);
		dsi_dev[sel]->dsi_inst_jump_sel1.dwval =
...
s32 dsi_open(u32 sel, disp_panel_para *panel)
{
	if (panel->lcd_fresh_mode == 0) {
		...
		if (panel->lcd_dsi_clk_mode == DISP_LCD_DSI_CLK_NON_CONTINUOUS)
			dsi_start(sel, DSI_START_HSTX_CLK_BREAK);
		/*
 ...
 * return ret: clk
 */
u32 dsi_comb_dphy_pll_set(__u32 sel, disp_panel_para *panel)
{
	u64 frq;
	u32 n;
	u32 div_p, div_m0, div_m1 = 0, div_m2 = 0, div_m3 = 0;
	u8 pixel_bits[3][4] = {
	    { 24, 24, 18, 16 }, { 24, 24, 16, 0 }, { 24, 24, 18, 16 } };

	frq = panel->lcd_dclk_freq * pixel_bits[panel->lcd_dsi_if][panel->lcd_dsi_format]
		/ panel->lcd_dsi_lane * 1000000; /* unit:Hz */
	if (frq <= 264000000) {
		frq = frq  * 8;
	...
	dphy_dev[sel]->dphy_pll_reg0.bits.n = n;
	dphy_dev[sel]->dphy_pll_reg0.bits.p = div_p;
	dphy_dev[sel]->dphy_pll_reg0.bits.m0 = div_m0;
	dphy_dev[sel]->dphy_pll_reg0.bits.m1 = div_m1; /* ?? */
	dphy_dev[sel]->dphy_pll_reg0.bits.post_div0_clk_ls = div_m2;
	dphy_dev[sel]->dphy_pll_reg0.bits.post_div1_clk_ls = div_m3;
	dphy_dev[sel]->dphy_pll_reg2.dwval = 0; /* Disable sdm */

	dphy_dev[sel]->dphy_pll_reg0.bits.pll_en = 1;
	dphy_dev[sel]->dphy_pll_reg0.bits.ldo_en = 1;

	dphy_dev[sel]->dphy_pll_reg1.bits.lockdet_en = 1;

	dphy_dev[sel]->dphy_pll_reg0.bits.reg_update = 1;
...
__s32 dsi_dphy_open(__u32 sel, disp_panel_para *panel)
{
	...
	dphy_dev[sel]->dphy_ana4.bits.reg_soft_rcal = 0x18;
	dphy_dev[sel]->dphy_ana4.bits.en_soft_rcal = 1;
	dphy_dev[sel]->dphy_ana4.bits.reg_vlv_set = 5;
	dphy_dev[sel]->dphy_ana4.bits.reg_vlptx_set = 3;
	dphy_dev[sel]->dphy_ana4.bits.reg_vtt_set = 6;
	dphy_dev[sel]->dphy_ana4.bits.reg_vres_set = 3;
	dphy_dev[sel]->dphy_ana4.bits.reg_ib = 4;
/*
...
*/
	...
	dphy_dev[sel]->dphy_ana0.bits.dsi_reg_lptx_setc = 7;
	dphy_dev[sel]->dphy_ana0.bits.dsi_reg_lptx_setr = 7;
...
*/
	...
	dsi_comb_dphy_pll_set(sel, panel);
...
__s32 lvds_combphy_open(__u32 sel, disp_panel_para *panel)
{
	...
	dphy_dev[sel]->combo_phy_reg1.dwval = 0x53; //0x43
...
u32 dsi_io_close(u32 sel)
{
	...
	dphy_dev[sel]->dphy_ana4.dwval = 0;
/*
...
*/
	...
	dphy_dev[sel]->dphy_ana0.bits.dsi_reg_lptx_setc = 0;
	dphy_dev[sel]->dphy_ana0.bits.dsi_reg_lptx_setr = 0;
/*
	dphy_dev[sel]->dphy_ana0.bits.reg_sfb = 0;
...
static s32 dsi_basic_cfg(u32 sel, disp_panel_para *panel)
{
	if (panel->lcd_dsi_if == LCD_DSI_IF_COMMAND_MODE) {
		...
		if (panel->lcd_dsi_data_mode == DISP_LCD_DSI_DATA_EOTP)
			dsi_dev[sel]->dsi_basic_ctl0.bits.hs_eotp_en = 1;
		else
	...
	dsi_dev[sel]->dsi_inst_func[DSI_INST_ID_DLY].bits.lane_den =
	    dsi_lane_den[panel->lcd_dsi_lane - 1];
	/* LP11 */
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_LP11_1-8].bits.instru_mode =
		DSI_INST_MODE_STOP;
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_LP11_1-8].bits.lane_cen = 1;
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_LP11_1-8].bits.lane_den =
		dsi_lane_den[panel->lcd_dsi_lane - 1];
	/* clock lane HS */
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_HSC_1-8].bits.instru_mode =
		DSI_INST_MODE_HS;
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_HSC_1-8].bits.trans_packet =
		DSI_INST_PACK_PIXEL;
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_HSC_1-8].bits.lane_cen	= 1;

	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_HSC_1-8].bits.lane_den	= 0;

	/* data lane initial skew calibration */

	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_DS_1-8].bits.instru_mode =
		DSI_INST_MODE_SCINIT;
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_DS_1-8].bits.trans_packet =
		DSI_INST_PACK_PIXEL;
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_DS_1-8].bits.lane_cen = 0;
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_DS_1-8].bits.lane_den =
		dsi_lane_den[panel->lcd_dsi_lane - 1];

	/* lane0 escape LPDT */
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_LPDT_1-8].bits.instru_mode =
		DSI_INST_MODE_ESCAPE;
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_LPDT_1-8].bits.escape_enrty =
		DSI_INST_ESCA_LPDT;
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_LPDT_1-8].bits.trans_packet =
		DSI_INST_PACK_COMMAND;
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_LPDT_1-8].bits.lane_cen = 0;
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_LPDT_1-8].bits.lane_den = 0x1;
	/* clock lane HS exit */
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_HSCEXIT_1-8].bits.instru_mode =
		DSI_INST_MODE_HSCEXIT;
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_HSCEXIT_1-8].bits.lane_cen = 1;
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_HSCEXIT_1-8].bits.lane_den = 0;
	/* data lane LP11 */
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_NOP_1-8].bits.instru_mode =
		DSI_INST_MODE_STOP;
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_NOP_1-8].bits.lane_cen	= 0;
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_NOP_1-8].bits.lane_den	=
		dsi_lane_den[panel->lcd_dsi_lane - 1];
	/* all lane NOP */
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_DLY_1-8].bits.instru_mode =
		DSI_INST_MODE_NOP;
	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_DLY_1-8].bits.lane_cen	= 1;

	dsi_dev[sel]->dsi_inst_func1[DSI_INST_ID_DLY_1-8].bits.lane_den	=
		dsi_lane_den[panel->lcd_dsi_lane - 1];

	dsi_dev[sel]->dsi_inst_loop_sel.dwval = 2 << (4 * DSI_INST_ID_LP11)
	    | 3 << (4 * DSI_INST_ID_DLY);

	dsi_dev[sel]->dsi_inst_loop_sel1.dwval = 2<<(4*(DSI_INST_ID_LP11_1-8))
...
static s32 dsi_packet_cfg(u32 sel, disp_panel_para *panel)
{
	...
	if ((panel->lcd_dsi_if == LCD_DSI_IF_VIDEO_MODE)
		...
		if (panel->lcd_dsi_if == LCD_DSI_IF_BURST_MODE) {
			...
			tmps = hspw / 2 * dsi_pixel_bits[format] / 8 - (4 + 4 + 2);
			dsi_hsa = tmps < 0 ? 0 : tmps;
			tmps = hbp / 2 * dsi_pixel_bits[format] / 8 - (4 + 4 + 2);
			dsi_hbp = tmps < 0 ? 0 : tmps;
			dsi_dev[sel]->dsi_pixel_ph.bits.wc = (panel->lcd_x / 2 + overlap) * \
				dsi_pixel_bits[panel->lcd_dsi_format] / 8;

			tmps = ((ht - hspw - hbp) / 2 - (x / 2 + overlap)) * 3 - 6 - 6;
			dsi_hfp = tmps < 0 ? 0 : tmps;
			tmps = (ht - hspw) / 2 * dsi_pixel_bits[format] / 8 - (4 + 4 + 2);
			dsi_hblk = tmps < 0 ? 0 : tmps;
			if (lane == 4) {
				tmps = ((ht / 2 - hspw / 2) * 3 - 10) / 2;  /* OK  */
				dsi_vblk = tmps < 0 ? 0 : tmps;
			}
			else
				dsi_vblk = 0;
			...
			dsi_hsa = tmps < 0 ? 0 : tmps;
			tmps = (hbp - hspw) * dsi_pixel_bits[format] / 8
			    - (4 + 2);
			dsi_hbp = tmps < 0 ? 0 : tmps;
			dsi_hact = x * dsi_pixel_bits[format] / 8;
			tmps = (ht - hspw) * dsi_pixel_bits[format] / 8
			    - (4 + 4 + 2);
			dsi_hblk = tmps < 0 ? 0 : tmps;
			tmps = dsi_hblk - (4 + dsi_hact + 2)
			    - (4 + dsi_hbp + 2);
			dsi_hfp = tmps < 0 ? 0 : tmps;
			if (lane == 4) {
				tmps = (ht * dsi_pixel_bits[format] / 8) * vt
				    - (4 + dsi_hblk + 2);
				tmp = tmps < 0 ? 0 : tmps;
