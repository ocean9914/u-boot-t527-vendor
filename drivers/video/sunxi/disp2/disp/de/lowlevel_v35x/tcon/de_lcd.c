...
extern __s32 lvds_combphy_open(__u32 sel, disp_panel_para *panel);
...
#if defined(CONFIG_MAH_SUN50IW6)
		lcd_top[0]->tcon_clk_gate.bits.hdmi_src = 0;
...
s32 tcon0_dsi_clk_source(u32 sel, disp_panel_para *panel)
{
	u32 en = 1;

	if (panel->lcd_dsi_clk_if)
		en = 0;
	if (sel == 0) {
		lcd_top[0]->tcon_clk_src.bits.lcd0_clk_src = en;
		lcd_top[0]->tcon_clk_src.bits.combo_phy0_clk_src = en;
		if (panel->lcd_tcon_mode == DISP_TCON_DUAL_DSI)
			lcd_top[0]->tcon_clk_src.bits.combo_phy1_clk_src = en;
	} else if (sel == 1) {
		lcd_top[0]->tcon_clk_src.bits.lcd1_clk_src = en;
		lcd_top[0]->tcon_clk_src.bits.combo_phy1_clk_src = en;
	}
	return 0;
}
...
s32 lvds_open(u32 sel, disp_panel_para *panel)
{
	...
	if (panel->lcd_lvds_if == LCD_LVDS_IF_DUAL_LINK) {
...
#else
		{
			...
			/*lcd_dev[sel]->tcon0_lvds_ana[sel].bits.denc = 1;*/
...
#if defined(SUPPORT_DSI) && !defined(DSI_VERSION_40) || defined(CONFIG_MACH_SUN55IW3)
...
s32 tcon0_set_dclk_div(u32 sel, u8 div)
{
	...
	/* lcd_dev[sel]->tcon0_dclk.bits.tcon0_dclk_div = 1; */
