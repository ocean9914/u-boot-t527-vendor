...
static struct disp_lcd_private_data *lcd_compat_private;
 __attribute__((unused)) static int g_disp_index;
static int g_compat_index;
static int g_lcd_compat_msg[10][2] = {0};
static u32 num_devices_support_lcd;
...
{
	return num_devices_support_lcd;
}

static int disp_check_timing_param(disp_panel_para *panel)
{
	u32 x = panel->lcd_x;
	u32 y = panel->lcd_y;
	u32 vt = panel->lcd_vt;
	...
	if (ht <= x + hbp) {
		...
		printf("[ERROR]timing param error!\n");
		printf("lcd_ht need to be equal to lcd_x + lcd_hbp + lcd_hfp.\n");
		printf("lcd_x = %d, lcd_hbp = %d, lcd_ht = %d, lcd_hspw = %d.\n", x, hbp, ht, hspw);
		printf("Now, reset lcd_ht = %u.\n\n", panel->lcd_ht);
	}

	if (vt <= y + vbp) {
		panel->lcd_vt = y + vbp + (vbp - panel->lcd_vspw);
		printf("[ERROR]timing param error!\n");
		printf("lcd_vt need to be equal to lcd_y + lcd_vbp + lcd_vfp.\n");
		printf("lcd_y = %d, lcd_vbp = %d, lcd_vt = %d, lcd_vspw = %d.\n", y, vbp, vt, vspw);
		printf("Now, reset lcd_vt = %u. \n\n", panel->lcd_vt);
	}
...
struct disp_device *disp_get_direct_lcd_compat(u32 disp, u32 index)
{
	...
	for (i = 0; i < disp; i++) {
		real_base_index += g_lcd_compat_msg[i][1];
	}

	real_base_index += index - 1;
	return &lcds_compat[real_base_index];
}
...
static struct disp_device *disp_get_lcd_compat(u32 disp, u32 index)
{
	...
	int i;
	for (i = 0; i < disp; i++) {
		num_compat += g_lcd_compat_msg[i][1];

	}
	return &lcds_compat[num_compat + index - 1];
}

struct disp_device* disp_get_lcd(u32 disp)
{
	u32 num_screens = disp_device_get_num();
#ifdef CONFIG_COMPATIBLE_PANEL
	u32 index = ((struct disp_lcd_private_data *)(lcds[disp].priv_data))->switch_to_compat_panel_index;
	u32 num_compat = disp_get_compat_lcd_panel_num(disp);
#endif

	if (disp >= num_screens || !bsp_disp_feat_is_supported_output_types(lcds[disp].hwdev_index, DISP_OUTPUT_TYPE_LCD)) {
...
int lcd_sys_script_get_item(char *main_name, char *sub_name, int value[], int type, int compat_idx)
{
	...
	primary_key1[8] = main_name[8];  // set /soc/lcd{disp}
	if (compat_idx == 0) {/*when using lcd0 or lcd1*/
		if (g_compat_index > 0 && main_name[8] == g_disp_index + '0')/* g_compat_index was set mean need to directly use lcd0_g_compat_index */
...
static s32 lcd_parse_panel_para(u32 disp, disp_panel_para *info, int compat_idx)
{
	...
	sprintf(primary_key, "/soc/lcd%d", disp);
	...
	if (info->lcd_if == 3) {
		...
		ret = lcd_sys_script_get_item(primary_key, "lcd_dsi_clk_mode", &value, 1, compat_idx);
		if (ret == 1)
			info->lcd_dsi_clk_mode = value;

		ret = lcd_sys_script_get_item(primary_key, "lcd_dsi_clk_if", &value, 1, compat_idx);
		if (ret == 1)
			info->lcd_dsi_clk_if = value;

		ret = lcd_sys_script_get_item(primary_key, "lcd_dsi_data_mode", &value, 1, compat_idx);
		if (ret == 1)
		...
		ret = lcd_sys_script_get_item(primary_key, "lcd_hv_data_polarity",
				     &value, 1, compat_idx);
		if (ret == 1)
	...
	ret = lcd_sys_script_get_item(primary_key, "lcd_start_delay", &value, 1, compat_idx);
	if (ret == 1)
	...
	ret = lcd_sys_script_get_item(primary_key, "lcd_fsync_pol", &value,
				     1, compat_idx);
	if (ret == 1)
...
#if defined(DE_VERSION_V35X) && defined(SUPPORT_DSI)
static s32 combphy_clk_config(struct clk *combphy_clk, disp_panel_para *panel)
{
	unsigned long combphy_rate = 0;
	unsigned long combphy_rate_set = 0;
	unsigned char bits_per_pixel = 0;
	...
	if (combphy_rate != combphy_rate_set)
		DE_WRN("combphy clk was setted rate  %lu, but real rate is %lu", combphy_rate, combphy_rate_set);
	return 0;
}
...
static s32 lcd_clk_enable(struct disp_device* lcd)
{
	...
	if (LCD_IF_LVDS == lcdp->panel_info.lcd_if) {
		...
		if ((LCD_LVDS_IF_DUAL_LINK == lcdp->panel_info.lcd_lvds_if ||
		     LCD_LVDS_IF_DUAL_LINK_SAME_SRC == lcdp->panel_info.lcd_lvds_if) &&
		    lcd->hwdev_index == 0) {
			dsi_num = 2;
...
combophy_enable:
		...
		for (i = dsi_num * lcd->disp, j = 0; i < CLK_DSI_NUM; ++i) {
			...
			if (ret) {
				DE_WRN("clk_prepare_enable for clk_mipi_dsi_combphy[%d] ,0x%p failed\n", i, lcdp->clk_mipi_dsi_combphy[i]);
				return ret;
			}
#endif
			if (j == dsi_num)
...
static s32 disp_lcd_pin_cfg(struct disp_device *lcd, u32 bon)
{
	...
	if (g_compat_index > 0)
		sprintf(dev_name, "/soc/lcd%d_%d", lcd->disp, g_compat_index);
	else if (lcdp->compat_panel_index > 0)
		sprintf(dev_name, "/soc/lcd%d_%d", lcd->disp, lcdp->compat_panel_index);
	else
...
{
	struct disp_device *lcd_compat = NULL;
	struct disp_lcd_private_data *lcdp;
	u32 num_compat;
	num_compat = disp_get_compat_lcd_panel_num(lcd->disp);
	...
	if (!lcd_compat)
		return 0;
	DE_INF("compat disp: %d, lcd_compat: %d \n", lcd_compat->disp, disp_lcd_get_priv(lcd_compat)->compat_panel_index);
	lcdp = disp_lcd_get_priv(lcd_compat);
	lcdp->need_open_again = true;

	disp_update_lcd_param(lcd->disp, index, SAVE_INDEX);
...
int disp_lcd_get_preview_device_info(struct disp_device *dispdev, struct disp_device_info *info)
{
	int ret = -1;
#ifdef SUPPORT_AMP_PREVIEW_REG
	struct disp_lcd_private_data *lcdp = disp_lcd_get_priv(dispdev);
	int start_delay = disp_al_lcd_get_start_delay(dispdev->hwdev_index, &lcdp->panel_info);
	...
	if (LCD_IF_DSI == lcdp->panel_info.lcd_if) {
		info->line_shift = DSI_LINE_SHIFT;
		info->line_mask = DSI_LINE_MASK;
		info->line_reg = DSI0_LINE_REG + DSI_REG_SIZE * dispdev->hwdev_index;
		info->safe_line = start_delay * 2 / 3;
		info->sleep_us = us_per_line;

	} else {
		return disp_device_get_preview_device_info(dispdev, info);
	}
	ret = 0;
#endif
	return ret;
...
static s32 disp_lcd_set_panel_funs(struct disp_device* lcd, char *name, disp_lcd_panel_fun * lcd_cfg)
{
	...
	sprintf(primary_key, "/soc/lcd%d", lcd->disp);

	ret = lcd_sys_script_get_item(primary_key, "lcd_driver_name",  (int *)drv_name, 2, g_compat_index);
	DE_INF("lcd %d, compat_panel_index: %u, driver_name %s,  panel_name %s\n", lcd->disp, lcdp->compat_panel_index, drv_name, name);
...
static s32 disp_lcd_timing_config(struct disp_video_timings *timings, disp_panel_para *panel_info)
{
			       ...
			       struct disp_device_config *config)
{
	...
	if (lcdp->panel_info.convert_info.is_enabled &&
		...
		const disp_panel_para_mapping *mapping = NULL;
...
s32 disp_init_lcd(disp_bsp_init_para * para)
{
	...
	for (hwdev_index=0; hwdev_index<num_devices; hwdev_index++) {
		...
		if (is_tcon_support_output_types(lcd->hwdev_index, TCON_TO_LVDS)) {
			if (lcd->hwdev_index == 4)
				lvds_clk_index = 1;
			if (lvds_clk_index >= DEVICE_LVDS_NUM)
				pr_err("lvds_clk_index >= DEVICE_LVDS_NUM");
			else {
				pr_info("lvds_clk_index = %d, disp = %d \n", lvds_clk_index, disp);
				lcdp->lvds_clk = para->mclk[DISP_MOD_LVDS + lvds_clk_index];
				lvds_clk_index++;
			}
		}
#else
		...
		for (i = index_base; i < CLK_DSI_NUM; ++i) {
			lcdp->dsi_clk[i] = para->mclk[DISP_MOD_DSI0 + i];
#if defined(DE_VERSION_V35X)
			lcdp->clk_mipi_dsi_combphy[i] = para->mclk[DSIP_MOD_COMBPHY0 + i];
#endif  /* DE_VERSION_V35X */
		}
		...
		lcd->get_preview_device_info = disp_lcd_get_preview_device_info;
	...
	disp = 0;
	i = 0;
	int curr_compat_index;
	DE_INF("num_devices: %d, num_compat: %d \n", num_devices, num_compat);
	for (hwdev_index = 0; hwdev_index < num_devices; hwdev_index++) {
		if (!bsp_disp_feat_is_supported_output_types(hwdev_index, DISP_OUTPUT_TYPE_LCD)) {
			continue;
		}
		sprintf(primary_key, "lcd%d", disp);
		printk("[DEBUG] primary_key: %s \n", primary_key);
		ret = disp_sys_script_get_item(primary_key, "status",
					       (int *)str, 2);
		if (ret != 2 || strncmp(str, "okay", 10) != 0) {
			printk("skip lcd%d \n", disp);
		...
		num_compat = disp_get_compat_lcd_panel_num(disp);

		DE_INF("num_compat is %d with lcd%d \n", num_compat, disp);
	for (curr_compat_index = 0; curr_compat_index < num_compat;  curr_compat_index++, i++) {
		lcd = &lcds_compat[i];
		lcdp = &lcd_compat_private[i];
		lcdp->compat_panel_index = curr_compat_index + 1;
		lcd->priv_data = (void *)lcdp;
...
#if defined(DE_VERSION_V35X)
			lcdp->clk_mipi_dsi_combphy[lcd->hwdev_index] = para->mclk[DSIP_MOD_COMBPHY0 + lcd->hwdev_index];
#endif  /* DE_VERSION_V35X */
		}
		DE_INF("total number of clk in dsi:%d\n", CLK_DSI_NUM);
#endif
